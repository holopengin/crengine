...
void addLineHorizontal( LVFormatter* fmt, int start, int end, int x, src_text_fragment_t * para, bool first, bool last, bool preFormattedOnly, bool isLastPara, bool hasInlineBoxes )
    {
        ...
        if ( fmt->m_initial_letter_exclusion.active && fmt->m_line_advance < fmt->m_initial_letter_exclusion.end_y
                                               && fmt->m_line_advance >= fmt->m_initial_letter_exclusion.start_y ) {
            // Avoid page split on every line actually affected by the exclusion.
            // (Using the exclusion start_y rather than the naive:
            //   (!first || fmt->m_initial_letter_exclusion.is_carried)
                    ...
                    }
                    {
                        ...
                        if ( word_is_exact_hanging && !vert_skip_draw ) {
                            ...
                            if ( y0 + (int)word->width > word_orig_clip.bottom
                                ...
                                word_clip.bottom =
                                    draw_extra_info->content_overflow_clip.bottom;
                                buf->SetClipRect(&word_clip);
                                restore_word_clip = true;
                                ltext_vert_exact_hanging_clip_recovery_count++;
                            }
                            lvRect active_clip;
                            buf->GetClipRect(&active_clip);
